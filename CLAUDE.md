...
- `tools/init.py` — `llama-tool.py init`. One-time host setup after cloning: `sudo cp`s `deploy/llama-server.service`, `deploy/llama-server@.service`, and `deploy/llama-server.logrotate` into `/etc/`, runs `systemctl daemon-reload` + `enable` (`llama-server.service` only — the `@.service` template is enabled per-instance by hand, since which presets should run as services is a per-host choice). Prints every command before running it and asks for confirmation (`-y`/`--dry-run`/`--no-enable` available). Deliberately does not start any service (nothing's installed yet on a fresh clone) or touch user/group membership (too host/GPU-specific to automate safely — stays a documented manual step).
