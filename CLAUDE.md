...
- `tools/init.py` — `llama-tool.py init`. One-time host setup after cloning: `sudo cp`s `deploy/llama-server.service`, `deploy/llama-server@.service`, `deploy/llama-server.logrotate`, and the `deploy/llama-server-restart[@].service`/`.timer` pairs into `/etc/`, runs `systemctl daemon-reload` + `enable` (`llama-server.service` only — the `@.service` template and both restart timers are enabled per-host/per-instance by hand, since which presets run as services, and whether periodic restarts are wanted at all, are per-host choices). Prints every command before running it and asks for confirmation (`-y`/`--dry-run`/`--no-enable` available). Deliberately does not start any service (nothing's installed yet on a fresh clone) or touch user/group membership (too host/GPU-specific to automate safely — stays a documented manual step).
...
- `deploy/llama-server.service` — systemd unit; ships with generic defaults (`User=1000`/`Group=1000` — systemd accepts a numeric id, and 1000 is the first regular user on most Debian/Ubuntu hosts — and `WorkingDirectory=/opt/llama-service`) that whoever deploys edits to match their actual user and clone path before running `init` (see README's "First-time setup"). Runs `llama-tool.py run` (no args, i.e. `$ACTIVE_PRESET`), logs appended to `llama-server.log`, `LimitMEMLOCK=infinity` for `--mlock` support, `MemorySwapMax=0` (cgroup v2) to stop the model/KV-cache from being swapped out. Relies on the `#!/usr/bin/env python3` shebang, so `llama-tool.py` must stay executable (`chmod +x`). Installed to `/etc/systemd/system/` by `llama-tool.py init`. Loads `deploy/llama-server.env` (gitignored, not installed by `init`) via `EnvironmentFile=` for secrets — see `llama.env`'s `LLAMA_API_KEY` note below.
- `deploy/llama-server@.service` — templated systemd unit for running several presets at once as independent services (each preset already owns its own `LLAMA_PORT`). Otherwise identical to `llama-server.service` above (same manual `User=`/`Group=`/`WorkingDirectory=` edit needed), except `ExecStart=llama-tool.py run %i` (runs the instance-name preset, e.g. `systemctl enable --now llama-server@qwen3-embedding-0.6b`) and per-instance logs at `llama-server-%i.log` instead of the shared `llama-server.log`. Installed alongside `llama-server.service` by `llama-tool.py init`, but never auto-enabled for any specific preset — that's a per-host choice.
- `deploy/llama-server.logrotate` — logrotate config covering both `llama-server.log` and `llama-server-*.log` (weekly, keep 4, compressed). Installed to `/etc/logrotate.d/llama-server` by `llama-tool.py init`.
- `deploy/llama-server-restart.service` + `deploy/llama-server-restart.timer` — oneshot `systemctl try-restart llama-server.service` fired daily at 03:00 (±5min randomized delay, `Persistent=true` to catch up after a missed/offline window), to bound memory growth from long uptimes. `try-restart` is a no-op if the target isn't running, so safe to leave enabled through maintenance windows; the target's own `Restart=always` (see `llama-server.service` above) is what actually brings it back up. Installed by `llama-tool.py init` but never auto-enabled — opt in with `systemctl enable --now llama-server-restart.timer`.
- `deploy/llama-server-restart@.service` + `deploy/llama-server-restart@.timer` — same idea, templated for `llama-server@%i.service` instances (`systemctl enable --now llama-server-restart@<preset>.timer`). Kept as a separate template rather than reusing the non-templated pair above because a plain (non-`@`) timer can't carry an instance name through to `try-restart llama-server@<preset>.service`.
...
Run several presets at once as separate systemd services (see `deploy/llama-server@.service` above):
```bash
